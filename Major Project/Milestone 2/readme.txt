...
The output file from the program will have the format "filename_data_R=rows_T=ticks.bin" and can be turned into a gif via ParallelViewer.py by changing the target FILE variable to the desired file name and the "row" variable to the appropriate file. Run this python script and view the output gif. WARNING FOR USE ONLY AS A TESTING TOOL ONLY, TURNING THE OUTPUT INTO A GIF REQUIRES A METRIC TONNE OF RAM, 32 GIGs WAS NOT ENOUGH TO GENERATE A GIF WITH ONLY 200 FRAMES.
