...
*
*/
void stream_to_file(int rowSize, int tick, std::string filename, int* state)
{
...
int main(int argc, char** argv)
{
    ...
    if (argc == 4) {
        ...
        fileName = std::string("data/") + "data_R=" + std::to_string(rowSize) + "_T=" + std::to_string(ticks) + ".bin";
    }
    ...
    */
    for (int i = 0; i < ticks; i++) {
        ...
        //print_state(state, rowSize);
        
        stream_to_file(rowSize, i, fileName, state);
    ...
    stream_to_file(rowSize, ticks, fileName, state);
